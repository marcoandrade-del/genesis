...
- **Falta:** (1) sessão OXY troca o stub do `backend.entidades()` por `GET /entidades` do :8080 (PR do dashboard) + e2e. Protocolo de commit: [[oxy-genesis-cada-um-comita]].
- **502 dos painéis por entidade RESOLVIDO (2026-07-16, oxy-repo #58, CI verde, 176 testes):** com entidade=unidade de BI, o dashboard manda `clienteId = entidadeId`, mas os 4 endpoints de apuração ao vivo (`serie-historica`, `detalhamento`, `saldo-bancario`, `guardiao` — os que passam pelo `EntidadeResolver`) estouravam **502 "Município desconhecido no Gênesis"** porque o resolver só mapeava município→PREFEITURA. Diagnóstico veio da sessão do oxy-dashboards (arquivo DIAGNOSE, na raiz do Gênesis). **Fix (só no oxy-bi-jpa):** `EntidadeResolver.entidade()` valida o `clienteId` contra o catálogo de ENTIDADES (`FonteEntidades`) e usa direto; fallback município→prefeitura mantido (ids disjuntos) p/ legado. Um ponto conserta os 4 — prefeitura/câmara/fundos/autarquias. Sem mudança no front nem no Gênesis. **Pendente:** a sessão OXY confere o e2e das telas de dados no browser (`:5174→:8080`). *Enforcement* por entidade (`entidades_permitidas` no JWT) segue independente/deferido.
