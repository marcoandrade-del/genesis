...
**⚠️ Refinamento (2026-07-16, onboarding de Criciúma/SC via SICONFI): o MODELO é COMPARTILHADO entre estados — não há necessariamente um por estado.** No dev: MS/AC/RO usam o MESMO `modeloContabil` `75bf5364-1d25-41d0-89b3-127534935418` (Naviraí é MS); PR usa `e17f30e5-...`; a maioria dos estados tem `estado.modeloContabilId = NULL`. O plano PCASP é NACIONAL (o `75bf5364` 2026 tem 8760 contas contábeis / 1808 receita / 3902 despesa, ~idêntico ao do PR). **Onboardar um estado novo = só `SET estado.modeloContabilId = <modelo compartilhado>`** (o `EntidadeService.criar` copia os planos do modelo; sem modelo → lança `ENTIDADE_NAO_PROCESSAVEL`). Marco: "os estados usam o mesmo modelo; use o do Naviraí [MS] para SC". Feito p/ SC. **O que É por-estado de fato: a tabela de eventos** (contabilização), que o conversor NÃO usa (ele grava previsão/dotação/razão de captura, não contabiliza via eventos) — por isso Criciúma importa 100% e fecha ao centavo sem tabela de eventos SC.
