...
### Cianorte/PR · Naviraí/MS · Vilhena/RO · Sarandi/PR (ELOTECH) — POPULADOS 2026-07-21 (#277)
Todos os 4 "vazios" eram **Elotech**. Import 100% pelo portal (`tce:'portal'`, execução do próprio portal — SEM PIT/SICONFI). Cianorte/Naviraí/Vilhena: receita ✅ + despesa(orç+exec) ✅ (Pref+Câmara+Previdência). Sarandi: receita ✅; despesa ⬜ (Elotech antigo `eloweb.net` v3.100, programática concatenada = follow-up). **Curitiba ELIMINADA** (era fixture de teste, não vazio; delete ordenado por FK: razão+relatórios+dashboard).
...
- ⬜ Elotech: importar os FUNDOS separados (Naviraí 12 fundos, Vilhena FMS/SAAE/Fundação) — a Pref id=1 não os consolida; + Sarandi despesa (parser do formato antigo via `inicio`/`tamanho`).
