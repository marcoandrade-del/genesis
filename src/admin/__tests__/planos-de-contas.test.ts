...
vi.mock('../../services/planos-de-contas.js', () => ({
  PlanosDeContasService: class {
...
vi.mock('../../services/importador-plano-contas.js', () => ({
  ImportadorPlanoContasService: class {
...
const MODELO = { id: 'm1', descricao: 'PCASP', ativo: true }
const PLANO = {
  id: 'p1', descricao: 'PCASP 2026', ano: 2026, modeloContabilId: 'm1',
  criadoEm: new Date(), atualizadoEm: new Date(),
}
const PLANO_COM_MODELO = { ...PLANO, modeloContabil: { id: 'm1', descricao: 'PCASP' }, _count: { contas: 0 } }
...
describe('adminPlanosDeContasRoutes', () => {
  ...
  beforeEach(async () => {
    ...
    ;({ app, prisma } = await criarApp({
      registrar: adminPlanosDeContasRoutes,
      comView: true,
  ...
  describe('GET /', () => {
    it('lista planos sem filtro', async () => {
      ...
      prisma.planoDeContas.findMany.mockResolvedValue([PLANO_COM_MODELO])
    ...
    it('estado vazio quando não há planos', async () => {
      ...
      prisma.planoDeContas.findMany.mockResolvedValue([])
      const res = await app.inject({ method: 'GET', url: '/' })
      expect(res.body).toContain('Nenhum plano de contas cadastrado')
    })
  })
  ...
  describe('GET /form', () => {
    it('renderiza form de novo plano com select de modelos ativos', async () => {
      ...
      expect(res.statusCode).toBe(200)
      expect(res.body).toContain('Novo Plano de Contas')
  ...
  describe('GET /:id/form', () => {
    ...
    it('renderiza form com modelo readonly quando edita', async () => {
      prisma.planoDeContas.findUnique.mockResolvedValue(PLANO_COM_MODELO)
      const res = await app.inject({ method: 'GET', url: '/p1/form' })
      expect(res.statusCode).toBe(200)
      expect(res.body).toContain('Editar Plano de Contas')
  ...
  describe('GET /:id/importar', () => {
    ...
    it('renderiza modal mostrando aviso se já tem contas', async () => {
      prisma.planoDeContas.findUnique.mockResolvedValue({
        ...PLANO_COM_MODELO, _count: { contas: 42 },
      })
      const res = await app.inject({ method: 'GET', url: '/p1/importar' })
      expect(res.statusCode).toBe(200)
      expect(res.body).toContain('Importar CSV de Contas')
  ...
  describe('POST /', () => {
    it('cria e redireciona com HX-Redirect', async () => {
      ...
      const res = await app.inject({
        method: 'POST', url: '/',
        ...form({ descricao: 'PCASP 2026', ano: '2026', modeloContabilId: 'm1' }),
      })
      expect(res.statusCode).toBe(204)
      expect(res.headers['hx-redirect']).toBe('/admin/planos-de-contas')
      expect(criarMock).toHaveBeenCalledWith({ descricao: 'PCASP 2026', ano: 2026, modeloContabilId: 'm1' })
    })
    ...
    it('mensagem default para erro não-Error', async () => {
      ...
      expect(res.body).toContain('Erro ao criar plano de contas')
    })
  })
  ...
  describe('PUT /:id', () => {
    it('atualiza descricao e ano', async () => {
      ...
      const res = await app.inject({
        method: 'PUT', url: '/p1', ...form({ descricao: 'PCASP novo', ano: '2027' }),
      })
      expect(res.statusCode).toBe(204)
      expect(res.headers['hx-redirect']).toBe('/admin/planos-de-contas')
      expect(atualizarMock).toHaveBeenCalledWith('p1', { descricao: 'PCASP novo', ano: 2027 })
    })

    it('re-renderiza com erro quando descrição vazia', async () => {
      prisma.planoDeContas.findUnique.mockResolvedValue(PLANO_COM_MODELO)
      const res = await app.inject({ method: 'PUT', url: '/p1', ...form({ descricao: '', ano: '2027' }) })
      expect(res.body).toContain('A descrição é obrigatória')
    ...
    it('re-renderiza com erro quando ano inválido', async () => {
      prisma.planoDeContas.findUnique.mockResolvedValue(PLANO_COM_MODELO)
      const res = await app.inject({ method: 'PUT', url: '/p1', ...form({ descricao: 'X', ano: 'foo' }) })
      expect(res.body).toContain('Ano inválido')
    })
    ...
    it('re-renderiza com erro do service', async () => {
      ...
      const res = await app.inject({ method: 'PUT', url: '/p1', ...form({ descricao: 'X', ano: '2027' }) })
    ...
    it('mensagem default para erro não-Error', async () => {
      ...
      const res = await app.inject({ method: 'PUT', url: '/p1', ...form({ descricao: 'X', ano: '2027' }) })
      expect(res.body).toContain('Erro ao atualizar plano de contas')
    })
  })
  ...
  describe('POST /:id/importar', () => {
    it('importa e devolve { data: { criadas } }', async () => {
      ...
      const res = await app.inject({
        method: 'POST', url: '/p1/importar', ...json({ csv: 'codigo,descricao,codigoPai,admiteMovimento\n1,A,,false' }),
      })
      expect(res.statusCode).toBe(200)
      expect(res.json()).toEqual({ data: { criadas: 100 } })
      expect(importarMock).toHaveBeenCalledWith('p1', expect.stringContaining('codigo,descricao'))
    })

    it('400 quando csv ausente (não-string)', async () => {
      const res = await app.inject({ method: 'POST', url: '/p1/importar', ...json({}) })
      expect(res.statusCode).toBe(400)
      expect(res.json()).toEqual({ error: { code: 'REQUISICAO_INVALIDA', message: 'CSV vazio.' } })
    })

    it('400 quando csv só com whitespace', async () => {
      const res = await app.inject({ method: 'POST', url: '/p1/importar', ...json({ csv: '   \n  ' }) })
      expect(res.statusCode).toBe(400)
    })

    it('propaga erro do importer via tratarErro (ErroNegocio → status apropriado)', async () => {
    ...
    it('propaga erro não-tratado como 500 do Fastify', async () => {
  ...
  describe('DELETE /:id', () => {
    it('exclui com 200', async () => {
      ...
      expect(res.statusCode).toBe(200)
      expect(excluirMock).toHaveBeenCalledWith('p1')
    })
