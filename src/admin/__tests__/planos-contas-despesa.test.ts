...
vi.mock('../../services/importador-plano-despesa.js', () => ({
  ImportadorPlanoDespesaService: class {
...
describe('adminPlanosContasDespesaRoutes', () => {
  ...
  describe('GET /:id/importar', () => {
    ...
    it('renderiza modal mostrando aviso se já tem contas', async () => {
      prisma.planoContasDespesa.findUnique.mockResolvedValue({ ...PLANO_COM_MODELO, _count: { contas: 42 } })
      const res = await app.inject({ method: 'GET', url: '/pd1/importar' })
      expect(res.statusCode).toBe(200)
      expect(res.body).toContain('Importar CSV de Contas da Despesa')
  ...
  describe('POST /:id/importar', () => {
    it('importa e devolve { data: { criadas } }', async () => {
      ...
      const res = await app.inject({
        method: 'POST', url: '/pd1/importar', ...json({ csv: 'codigo,descricao,codigoPai,admiteMovimento\n3,A,,false' }),
      })
      expect(res.statusCode).toBe(200)
      expect(res.json()).toEqual({ data: { criadas: 100 } })
      expect(importarMock).toHaveBeenCalledWith('pd1', expect.stringContaining('codigo,descricao'))
    })

    it('400 quando csv ausente (não-string)', async () => {
      const res = await app.inject({ method: 'POST', url: '/pd1/importar', ...json({}) })
      expect(res.statusCode).toBe(400)
      expect(res.json()).toEqual({ error: { code: 'REQUISICAO_INVALIDA', message: 'CSV vazio.' } })
    })

    it('400 quando csv só com whitespace', async () => {
      const res = await app.inject({ method: 'POST', url: '/pd1/importar', ...json({ csv: '   \n  ' }) })
      expect(res.statusCode).toBe(400)
    })
