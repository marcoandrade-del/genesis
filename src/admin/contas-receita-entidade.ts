...
export async function adminContasReceitaEntidadeRoutes(app: FastifyInstance) {
  const service = new ContasReceitaEntidadeService(app.prisma)
  ...
  async function comTemFilhos<T extends { id: string }>(contas: T[]): Promise<(T & { temFilhos: boolean })[]> {
    ...
    const grupos = await app.prisma.contaReceitaEntidade.groupBy({
      by: ['parentId'],
  ...
  app.get<{ Querystring: { entidadeId?: string; ano?: string } }>('/', async (req, reply) => {
    ...
    if (!entidadeId) {
      return reply.view('contas-receita-entidade/index', {
        title: 'Contas da Receita (Entidade) — Gênesis Admin',
        active: 'contas-receita-entidade', userEmail: req.user.email,
  ...
  app.get<{ Params: { id: string } }>('/:id/filhos', async (req, reply) => {
    ...
    return reply.view('contas-receita-entidade/filhos', { filhos })
  })
  ...
  app.get<{ Params: { id: string } }>('/:id/desdobrar', async (req, reply) => {
    ...
    return reply.view('contas-receita-entidade/desdobrar', { conta, sugestao, erro: null })
  })
  ...
  app.post<{ Params: { id: string }; Body: { codigo: string; descricao: string } }>(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        return reply
          .header('HX-Redirect', `/admin/contas-receita-entidade?entidadeId=${filho.entidadeId}&ano=${filho.ano}`)
          .status(204).send()
      } catch (e: unknown) {
        const conta = await service.buscarPorId(req.params.id)
        if (!conta) return reply.status(404).send('Conta não encontrada.')
        ...
        return reply.view('contas-receita-entidade/desdobrar', { conta, sugestao, erro })
      }
    },
  )
  ...
  app.delete<{ Params: { id: string } }>('/:id', async (req, reply) => {
    try {
      ...
      return reply
        .header('HX-Redirect', `/admin/contas-receita-entidade?entidadeId=${conta.entidadeId}&ano=${conta.ano}`)
        .status(204)
        .send()
    } catch (e: unknown) {
