...
export async function adminPlanosContasReceitaRoutes(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { modeloContabilId?: string } }>('/', async (req, reply) => {
    ...
    return reply.view(
      'planos-contas-receita/index',
      {
        title: 'Planos de Contas da Receita — Gênesis Admin',
        active: 'planos-contas-receita',
  ...
  app.get('/form', async (_req, reply) => {
    ...
    return reply.view('planos-contas-receita/form', { plano: null, modelos, erro: null })
  })
  ...
  app.post<{ Body: { descricao: string; ano: string; modeloContabilId: string } }>(
    ...
    async (req, reply) => {
      ...
      const reRenderErro = async (erro: string) => {
        ...
        return reply.view('planos-contas-receita/form', { plano: null, modelos, erro })
      }
      ...
      try {
        ...
        return reply.header('HX-Redirect', '/admin/planos-contas-receita').status(204).send()
      } catch (e: unknown) {
        return reRenderErro(e instanceof Error ? e.message : 'Erro ao criar plano de contas da receita.')
      }
    },
  )
  ...
  app.put<{ Params: { id: string }; Body: { descricao: string; ano: string } }>(
    ...
    async (req, reply) => {
      ...
      const reRenderErro = async (erro: string) => {
        ...
        return reply.view('planos-contas-receita/form', { plano, modelos: [], erro })
      }
      if (!descricao?.trim()) return reRenderErro('A descrição é obrigatória.')
      const anoNum = parseInt(ano, 10)
      if (Number.isNaN(anoNum) || anoNum < 1900 || anoNum > 9999) {
      ...
      try {
        ...
        return reRenderErro(e instanceof Error ? e.message : 'Erro ao atualizar plano de contas da receita.')
      }
    },
  )
