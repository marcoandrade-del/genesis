...
export async function adminUnidadesOrcamentariaRoutes(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { entidadeId?: string } }>('/form', async (req, reply) => {
    ...
    return reply.view('unidades-orcamentaria/form', { uo: null, entidadeId, erro: null })
  })
  ...
  app.get<{ Params: { id: string } }>('/:id/form', async (req, reply) => {
    ...
    return reply.view('unidades-orcamentaria/form', { uo, entidadeId: uo.entidadeId, erro: null })
  })

  // ── CREATE ──────────────────────────────────────────────────────────────────
  app.post<{ Body: { entidadeId: string; codigo: string; nome: string; ativa?: string } }>(
    '/',
    async (req, reply) => {
      const { entidadeId, codigo, nome, ativa } = req.body
      const reRender = (erro: string) =>
        reply.view('unidades-orcamentaria/form', { uo: null, entidadeId, erro })
      ...
      try {
        await service.criar(entidadeId, {
          codigo,
          nome,
          ativa: ativa !== 'false',
        })
  ...
  // ── UPDATE ──────────────────────────────────────────────────────────────────
  app.put<{ Params: { id: string }; Body: { codigo: string; nome: string; ativa?: string } }>(
    '/:id',
    async (req, reply) => {
      const { codigo, nome, ativa } = req.body
      const uo = await app.prisma.unidadeOrcamentaria.findUnique({ where: { id: req.params.id } })
      if (!uo) return reply.status(404).send('Unidade orçamentária não encontrada.')
      const reRender = (erro: string) =>
        reply.view('unidades-orcamentaria/form', { uo, entidadeId: uo.entidadeId, erro })
