...
describe('ModulosService.excluir', () => {
  ...
  it('lança RECURSO_NAO_ENCONTRADO quando módulo não existe', async () => {
    ...
    await expect(service.excluir('m-inexistente')).rejects.toMatchObject({ code: 'RECURSO_NAO_ENCONTRADO' })
  })

  it('inicia transação para excluir em cascata quando módulo existe', async () => {
