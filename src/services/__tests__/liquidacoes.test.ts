...
function mockEmpenho(over: Partial<Record<string, unknown>> = {}) {
  prisma.empenho.findUnique.mockResolvedValue({ id: 'e1', entidadeId: 'ent1', status: 'ATIVO', valor: '500', valorLiquidado: '100', ...over })
}
...
describe('LiquidacoesService.criar', () => {
  ...
  it('liquida e incrementa o valorLiquidado do empenho', async () => {
    ...
    await service.criar('ent1', dadosOk({ valor: '400' })) // = disponível
...
describe('LiquidacoesService.cancelar', () => {
  ...
  it('cancela e estorna o liquidado', async () => {
    prisma.liquidacao.findUnique.mockResolvedValue({ id: 'l1', status: 'ATIVA', valorPago: '0', empenhoId: 'e1', valor: new Prisma.Decimal('300') })
    prisma.liquidacao.update.mockResolvedValue({ id: 'l1', status: 'CANCELADA' })
    await service.cancelar('l1')
