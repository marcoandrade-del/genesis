...
import { NIVEL_MAX_RECEITA } from './contas-receita.js'

/**
 * Importador do Plano de Contas da Receita via CSV. Mesmo formato e validações
 * do importador contábil (colunas codigo/descricao/codigoPai/admiteMovimento),
 * com o teto de níveis próprio da receita.
 */
export class ImportadorPlanoReceitaService {
  constructor(private prisma: PrismaClient) {}

  async importar(planoId: string, csv: string): Promise<{ criadas: number }> {
    const plano = await this.prisma.planoContasReceita.findUnique({ where: { id: planoId } })
    ...
    const niveis = validar(linhas, NIVEL_MAX_RECEITA)
    ...
    try {
      const { count } = await this.prisma.contaReceita.createMany({ data: dados })
