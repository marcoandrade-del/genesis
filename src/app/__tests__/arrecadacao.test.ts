...
const { listarMock, criarMock, resumoMock, prevListarMock } = vi.hoisted(() => ({
  listarMock: vi.fn(),
  criarMock: vi.fn(),
  resumoMock: vi.fn(),
...
describe('appArrecadacaoRoutes', () => {
  ...
  beforeEach(async () => {
    ;[listarMock, criarMock, resumoMock, prevListarMock].forEach((m) => m.mockReset())
  ...
  it('POST registra o movimento e redireciona', async () => {
    ...
    expect(criarMock).toHaveBeenCalledWith('o1', DADOS)
  })
  ...
  it('POST sem corpo monta dados vazios (validação fica no service)', async () => {
    ...
    expect(res.statusCode).toBe(302)
    expect(criarMock).toHaveBeenCalledWith('o1', { previsaoId: '', tipo: '', data: '', valor: '', historico: '' })
  })
