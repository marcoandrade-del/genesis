...
export async function appSincronizacaoRoutes(app: FastifyInstance) {
  ...
  async function render(req: FastifyRequest, reply: FastifyReply, entidade: unknown, opts: { aviso?: string; erro?: string; status?: number } = {}) {
    ...
    const execucoes = await app.prisma.sincronizacaoPortal.findMany({
      where: { entidadeId },
  ...
  app.post('/sincronizacao/rodar', async (req, reply) => {
    ...
    void (async () => {
      try {
        // ordem: RECEITA sempre antes da DESPESA
    ...
    return render(req, reply, entidade, {
      aviso: `Sincronização de ${String(mes).padStart(2, '0')}/${ano} iniciada (receita → despesa). A varredura do portal leva alguns minutos — atualize a página para ver o resultado no log.`,
    })
  })
}
